...
Opire 在创建奖励时提供了完全的灵活性，无论您是项目维护者还是感兴趣的用户。

如果奖励创建者与项目无关（不是托管项目代码的组织成员之一），则在支付奖励时会收取额外的 [百分比](/rewards/pricing#percentage-for-code-owner)，并且这些百分比将进入代码所有者的手中。

另一方面，如果奖励创建者是项目成员，则最终成本不会应用此百分比。
...
flowchart TD
    ...
    通过Stripe支付 --> 创建者是否为组织成员{Is creator a member<br/> of the organization?}:::opire -->  创建者不是[No] --> 代码所有者费用(Code Owner Fee):::codeowner
