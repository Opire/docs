...
Generally, the total cost to be paid will consist of the reward amount, plus the **Stripe fee ($0.35 + 5.25%)** and **our fee (10% of the reward)**.

### Percentage for Code Owner

Rewards can be created by any individual/organization. Therefore, code owners (or maintainers) can also be the creators of rewards. When this happens, the associated costs remain unchanged from what was seen in the previous section.

However, when the reward creator is someone external to the organization, the associated costs will include a **5% of the reward** that will go entirely to the organization/person maintaining the project where the reward was created.

<br/>
<InfoPanel>
At Opire, we are committed to open-source software, so we believe that **its maintainers should be rewarded economically** for their significant effort and influence.

Ideally, we would like to see a world where **anyone can make a living from open-source software** (both open-source maintainers and developers who collaborate with them).
</InfoPanel>
<br/>
