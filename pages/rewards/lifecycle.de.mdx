...
Bei Opire beginnt der Lebenszyklus einer Belohnung, wenn ein Benutzer die Lösung eines Problems anregen möchte. Dieser Prozess beginnt damit, eine Belohnung zu erstellen, die mit einem Problem verknüpft ist, indem [unsere Befehle](/overview/commands#create-reward) verwendet werden.

Nachdem die Belohnung erstellt wurde, erscheint sie auf der [Opire-Plattform](https://app.opire.dev) sowie auf dem Problem selbst (mit einer Nachricht von unserem Bot und einem Label, das den Betrag der Belohnung angibt). Jeder Programmierer kann die Belohnung anzeigen und entscheiden, ob er versuchen möchte, das Problem zu lösen, und andere mit dem [Versuch](/overview/commands#try-to-solve-the-issue) -Befehl benachrichtigen.
...
## Ersteller nicht mit Organisation verbunden

Opire bietet vollständige Flexibilität bei der Erstellung von Belohnungen, egal ob Sie einer der Projektbetreuer oder ein interessierter Benutzer sind.

Wenn der Belohnungsersteller extern zum Projekt gehört (nicht zu den Mitgliedern der Organisation gehört, die den Projektcode hostet), wird bei der Zahlung der Belohnung ein [zusätzlicher Prozentsatz](/rewards/pricing#percentage-for-code-owner) berechnet, der an die Code-Besitzer geht.

Andererseits wird dieser Prozentsatz nicht auf die endgültigen Kosten angewendet, wenn der Belohnungsersteller ein Projektmitglied ist.
...
flowchart TD
    ...
    pay_from_stripe --> creator_is_member{Ist der Ersteller Mitglied<br/> der Organisation?}:::opire -->  creator_is_not_member[Nein] --> codeowner_fee(Code-Besitzer Gebühr):::codeowner
