...
At Opire, the lifecycle of a reward begins when a user wants to incentivize the resolution of an issue. This process starts by creating a reward associated with an issue using [our commands](/overview/commands#create-reward).

Upon creating the reward, it will appear on the [Opire platform](https://app.opire.dev), as well as on the issue itself (with a message from our bot and a label indicating the reward amount). Any programmer can view the reward and decide whether to attempt to resolve the issue, notifying others using the [try](/overview/commands#try-to-solve-the-issue) command.
...
## Creator Unrelated to Organization

Opire provides complete flexibility when creating rewards, whether you are one of the project maintainers or an interested user.

If the reward creator is external to the project (not among the members of the organization hosting the project code), an [additional percentage](/rewards/pricing#percentage-for-code-owner) will be charged when paying the reward, and it will go to the code owners.

On the other hand, if the reward creator is a project member this percentage will not be applied to the final costs.
...
flowchart TD
    ...
    pay_from_stripe --> creator_is_member{Is creator a member<br/> of the organization?}:::opire -->  creator_is_not_member[No] --> codeowner_fee(Code Owner Fee):::codeowner
