...
In Opire, the lifecycle of a tip begins when a creator decides to give a tip to a user. This process starts by creating a tip associated with an Issue or Pull Request (PR) using [our commands](/overview/commands#tip-a-user).
...
If the user can receive payments, Opire redirects the creator to a Stripe payment gateway. The final amount to be paid will be the tip amount plus the [associated costs](/tips/pricing). After completing the payment, the user receives the tip within approximately 1-7 business days.
